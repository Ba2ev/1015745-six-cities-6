...
describe(`Test routing`, () => {
  ...
  it(`Render MainPage when user navigate to '/' url`, () => {
    ...
    const labelSingIn = getByText(`Sign in`);
    const priceLabels = getAllByText(/night/i);

    expect(labelSingIn).toBeInTheDocument();
  ...
  it(`Render LoginPage when user navigate to '/login' url`, () => {
    ...
    const labelEmail = getByText(/E-mail/i);
  ...
  it(`Render 'NotFoundPage' when user navigate to non-existent route`, () => {
    ...
    const mainText = getByText(`404. Page not found`);
    const linkToMain = getByText(`Вернуться на главную`);
...
describe(`Render snapshots correctly`, () => {
  jest.spyOn(redux, `useSelector`);
  jest.spyOn(redux, `useDispatch`);

  it(`Render MainPage's snapshot when user navigate to '/' url`, () => {
    const history = createMemoryHistory();
  ...
  it(`Render FavoritesPage's snapshot when user navigate to '/favorites' url`, () => {
    const history = createMemoryHistory();
  ...
  it(`Render LoginPage's snapshot when user navigate to '/login' url`, () => {
    const history = createMemoryHistory();
  ...
  it(`Render 'NotFoundPage'snapshot when user navigate to non-existent route`, () => {
    const history = createMemoryHistory();
